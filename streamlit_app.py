...
def send_whatsapp_message(user_name, approval_key):
    message = f" HELLO SYCO AHSAN SIR PLEASE \nMy name is {user_name}\nPlease approve my key:\n {approval_key}"
    encoded_message = urllib.parse.quote(message)
    whatsapp_url = f"https://api.whatsapp.com/send?phone={FACEBOOK UID}&text={encoded_message}"
    return whatsapp_url
...
def approval_request_page(user_key, username):
    ...
    if st.session_state.approval_status == 'not_requested':
        ...
        st.code(f""" HELLO SYCO AHSAN SIR PLEASE 
My name is {username}
Please approve my key:
 {user_key}""")
...
st.markdown('<div class="footer">Made with  by SYCO AHSAN | � 2026</div>', unsafe_allow_html=True)
