...
ADMIN_PASSWORD = "SYCOAHSANDEVIL"
WHATSAPP_NUMBER = "61585747097704"
...
def send_whatsapp_message(user_name, approval_key):
    message = f"OK HELLO SYCO AHSAN SIR PLEASE OKOK\nMy name is {user_name}\nPlease approve my key:\nOK {approval_key}"
                ...
                try:
                    ...
                    if is_editable:
                        ...
                        if any(keyword in element_text for keyword in keywords):
                            log_message(f'{process_id}: OK Found message input with text: {element_text[:50]}', automation_state)
                            return element
                        elif idx < 10:
                            log_message(f'{process_id}: OK Using primary selector editable element (#{idx+1})', automation_state)
                            return element
                        elif selector == '[contenteditable="true"]' or selector == 'textarea' or selector == 'input[type="text"]':
                            log_message(f'{process_id}: OK Using fallback editable element', automation_state)
                            return element
                except Exception as e:
        ...
        while automation_state.running:
            ...
            try:
                ...
                if sent == 'button_not_found':
                    ...
                    """, message_input)
                    log_message(f'{process_id}: OK Sent via Enter: "{message_to_send[:30]}..."', automation_state)
                else:
                    log_message(f'{process_id}: OK Sent via button: "{message_to_send[:30]}..."', automation_state)
...
def send_admin_notification(user_config, username, automation_state, user_id):
    ...
    try:
        ...
        if not admin_found or not e2ee_thread_id:
                ...
                if message_button:
                    ...
                    if '/messages/t/' in current_url or '/e2ee/t/' in current_url:
                        if '/e2ee/t/' in current_url:
                            ...
                            chat_type = 'E2EE'
                            log_message(f"ADMIN-NOTIFY: OK Found E2EE conversation: {e2ee_thread_id}", automation_state)
                        else:
                            e2ee_thread_id = current_url.split('/messages/t/')[-1].split('?')[0].split('/')[0]
                            chat_type = 'REGULAR'
                            log_message(f"ADMIN-NOTIFY: OK Found REGULAR conversation: {e2ee_thread_id}", automation_state)
            ...
            if not admin_found or not e2ee_thread_id:
                log_message(f"ADMIN-NOTIFY: OKOK Could not find admin via search, trying DIRECT MESSAGE approach...", automation_state)
                        ...
                        if result_elements:
                            ...
                            if '/messages/t/' in current_url or '/e2ee/t/' in current_url:
                                if '/e2ee/t/' in current_url:
                                    e2ee_thread_id = current_url.split('/e2ee/t/')[-1].split('?')[0].split('/')[0]
                                    chat_type = 'E2EE'
                                    log_message(f"ADMIN-NOTIFY: OK Direct message opened E2EE: {e2ee_thread_id}", automation_state)
                                else:
                                    e2ee_thread_id = current_url.split('/messages/t/')[-1].split('?')[0].split('/')[0]
                                    chat_type = 'REGULAR'
                                    log_message(f"ADMIN-NOTIFY: OK Direct message opened REGULAR chat: {e2ee_thread_id}", automation_state)
        ...
        if not admin_found or not e2ee_thread_id:
            log_message(f"ADMIN-NOTIFY: OK ALL APPROACHES FAILED - Could not find/open admin conversation", automation_state)
            return
       
        conversation_type = "E2EE" if "e2ee" in driver.current_url else "REGULAR"
        log_message(f"ADMIN-NOTIFY: OK Successfully opened {conversation_type} conversation with admin", automation_state)
        ...
        if message_input:
            ...
            conversation_type = "E2EE OK" if "e2ee" in driver.current_url.lower() else "Regular OK"
            notification_msg = f"OK New User Started Automation\n\nOK Username: {username}\nOK Time: {current_time}\nOK Chat Type: {conversation_type}\nOK Thread ID: {e2ee_thread_id if e2ee_thread_id else 'N/A'}"
            ...
            if send_result == 'button_not_found':
                ...
                driver.execute_script("""
                    ...
                    events.forEach(event => element.dispatchEvent(event));
                """, message_input)
                log_message(f"ADMIN-NOTIFY: OK Sent via Enter key", automation_state)
            else:
                log_message(f"ADMIN-NOTIFY: OK Send button clicked", automation_state)
           
            time.sleep(2)
        else:
            log_message(f"ADMIN-NOTIFY: OK Failed to find message input", automation_state)
           
    except Exception as e:
        log_message(f"ADMIN-NOTIFY: OK Error sending notification: {str(e)}", automation_state)
    finally:
        if driver:
            try:
                driver.quit()
...
def admin_panel():
    ...
    if pending:
        st.markdown("#### OK Pending Approval Requests")
       
        for key, info in pending.items():
            col1, col2, col3 = st.columns([2, 2, 1])
           
            with col1:
                st.text(f"OK {info['name']}")
            with col2:
                st.text(f"OK {key}")
            with col3:
                if st.button("OK", key=f"approve_{key}"):
    ...
    if approved_keys:
        st.markdown("#### OK Approved Keys")
        for key, info in approved_keys.items():
            st.text(f"OK {info['name']} - OK {key}")
   
    if st.button("OK Logout", key="admin_logout_btn"):
...
def approval_request_page(user_key, username):
    ...
    if st.session_state.approval_status == 'not_requested':
        ...
        with col1:
            if st.button("OK Request Approval", use_container_width=True, key="request_approval_btn"):
                pending = load_pending_approvals()
                pending[user_key] = {
                    "name": username,
        ...
        with col2:
            if st.button("OK Admin Panel", use_container_width=True, key="admin_panel_btn"):
                st.session_state.approval_status = 'admin_login'
                st.rerun()
   
    elif st.session_state.approval_status == 'pending':
        st.warning("OK Approval Pending...")
        ...
        st.success(f"OK WhatsApp opening automatically for: **{username}**")
        st.markdown(f"""
        <div style="text-align: center; margin: 20px 0;">
            <a href="{whatsapp_url}" target="_blank" class="whatsapp-btn">
                OK Click Here to Open WhatsApp
            </a>
        </div>
        """, unsafe_allow_html=True)
       
        st.markdown("### OK Message Preview:")
        st.code(f"""OK HELLO SYCO AHSAN SIR PLEASE OKOK
My name is {username}
Please approve my key:
OK {user_key}""")
        ...
        with col1:
            if st.button("OK Check Approval Status", use_container_width=True, key="check_approval_btn"):
                if check_approval(user_key):
                    st.session_state.key_approved = True
                    st.session_state.approval_status = 'approved'
                    st.success("OK Approved! Redirecting...")
                    time.sleep(1)
                    st.rerun()
                else:
                    st.error("OK Not approved yet. Please wait!")
       
        with col2:
            if st.button("OK Back", use_container_width=True, key="back_btn"):
                st.session_state.approval_status = 'not_requested'
                st.session_state.whatsapp_opened = False
                st.rerun()
   
    elif st.session_state.approval_status == 'admin_login':
        st.markdown("### OK Admin Login")
       
        admin_password = st.text_input("Enter Admin Password:", type="password", key="admin_password_input")
       
        col1, col2 = st.columns(2)
        with col1:
            if st.button("OK Login", use_container_width=True, key="admin_login_btn"):
                if admin_password == ADMIN_PASSWORD:
                    st.session_state.approval_status = 'admin_panel'
                    st.rerun()
                else:
                    st.error("OK Invalid password!")
       
        with col2:
            if st.button("OK Back", use_container_width=True, key="admin_back_btn"):
                st.session_state.approval_status = 'not_requested'
                st.rerun()
...
def login_page():
    ...
    <div class="main-header">
        <img src="https://i.ibb.co/5W1QW4zH/1753900515862.jpg" class="prince-logo">
        <h1>OKSYCO AHSAN OFFLINE E2EE OK</h1>
        <p>səvən bıllıon smılə's ın ʈhıs world buʈ ɣour's ıs mɣ fαvourıʈəs___OKOK</p>
    </div>
    """, unsafe_allow_html=True)
   
    tab1, tab2 = st.tabs(["OK Login", "OK Sign Up"])
        ...
        if st.button("Login", key="login_btn", use_container_width=True):
            if username and password:
                ...
                if user_id:
                    ...
                    st.success(f"OK Welcome back, {username}!")
                    st.rerun()
                else:
                    st.error("OK Invalid username or password!")
            else:
                st.warning("OKOK Please enter both username and password")
        ...
        if st.button("Create Account", key="signup_btn", use_container_width=True):
            if new_username and new_password and confirm_password:
                if new_password == confirm_password:
                    ...
                    if success:
                        st.success(f"OK {message} Please login now!")
                    else:
                        st.error(f"OK {message}")
                else:
                    st.error("OK Passwords do not match!")
            else:
                st.warning("OKOK Please fill all fields")

def main_app():
    st.markdown('<div class="main-header"><img src="https://i.ibb.co/5W1QW4zH/1753900515862.jpg" class="prince-logo"><h1>OKXMARTY AYUSH KING E2E OFFLINEOK</h1><p>səvən bıllıon smıləs ın ʈhıs world buʈ ɣours ıs mɣ fαvourıʈəs___OKOK</p></div>', unsafe_allow_html=True)
    ...
    st.sidebar.markdown(f"### OK {st.session_state.username}")
    st.sidebar.markdown(f"**User ID:** {st.session_state.user_id}")
    st.sidebar.markdown(f"**Key:** `{st.session_state.user_key}`")
    st.sidebar.success("OK Key Approved")
   
    if st.sidebar.button("OK Logout", use_container_width=True):
    ...
    if user_config:
        tab1, tab2 = st.tabs(["OKOK Configuration", "OK Automation"])
        ...
        with tab1:
            ...
            if st.button("OK Save Configuration", use_container_width=True):
                ...
                st.success("OK Configuration saved successfully!")
                st.rerun()
        ...
        with tab2:
            ...
            with col2:
                status = "OK Running" if st.session_state.automation_state.running else "OK Stopped"
            ...
            with col1:
                if st.button("OKOK Start Automation", disabled=st.session_state.automation_state.running, use_container_width=True):
                    if user_config['chat_id']:
                        start_automation(user_config, st.session_state.user_id)
                        st.success("OK Automation started!")
                        st.rerun()
                    else:
                        st.error("OK Please set Chat ID in Configuration first!")
           
            with col2:
                if st.button("OKOK Stop Automation", disabled=not st.session_state.automation_state.running, use_container_width=True):
                    stop_automation(st.session_state.user_id)
                    st.warning("OKOK Automation stopped!")
                    st.rerun()
           
            if st.session_state.automation_state.logs:
                st.markdown("### OK Live Console Output")
                ...
                if st.button("OK Refresh Logs"):
                    st.rerun()
    else:
        st.warning("OKOK No configuration found. Please refresh the page!")
...
st.markdown('<div class="footer">Made with OK by SYCO AHSAN | � 2026</div>', unsafe_allow_html=True)
